...
<?php
/*
get_subscription_9-4.php replaces get_subscription2.php.  The old version must be
left around for existing programs to call.

The new version adds support for evaluation activations.  Corresponding
changes were made in the activation programs for release 9.4.

Note, all the messages sent back are denoted by : for easy parsing on the activate.cpp side.
*/
...
/* all from amzi_connect now ...
	
$mailee = 'amzi.dennis@gmail.com';
//mail($mailee, 'get_subscription_9-4', 'Getting started', '');

// Connect to the licenses database
$link = mysql_connect('localhost', 'root', 'galax');
if (!$link) {
	$msg = ':ERROR: Cannot connect to subscription database -- ' . mysql_error() . ' -- Please contact Amzi! Tech Support.:';
	mail($mailee, 'ERROR: mysql_connect', $msg, '');
    die($msg);
}

//mail($mailee, 'Diagnostic 2', 'MySql connected', '');  // got here

// Make licenses the current db
$db_selected = mysql_select_db('subscriptions', $link);
if (!$db_selected) {
	$msg = ':ERROR: Cannot use subscriptions database -- ' . mysql_error() . ' -- Please contact Amzi! Tech Support.:';
	mail($mailee, 'ERROR: mysql_selectdb', $msg, '');
	mysql_close($link);
    die($msg);
}

//mail($mailee, 'Diagnostic 3', 'subscriptions DB selected', '');  // got here
*/
...
if ($fact == "yes") mail($mailee, 'get_subscription_9-4', $addr . " " . $host . "\n" . $msg, '');
...
if ($dbenterpriseName == "self") {
	...
	mail($mailee,
		"Activation 9-4: " . $dbenterpriseName . " -- " . $fusername,
		$query . "\n",
		'');
}
